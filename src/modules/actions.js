...
export function fetchSkincareProducts(
  page,
  productPerPage,
  category,
  filter,
...
export const FETCH_SKINCARE_PRODUCTS_SUCCESS =
  "FETCH_SKINCARE_PRODUCTS_SUCCESS";
export const fetchSkincareProductsSuccess = (
  products,
  numberOfProducts,
  brands
) => ({
...
export const fetchSchedules = (service_id, staff_id, id) => (dispatch) => {
  axios
    .get("/scheduleDates/" + service_id + "/" + id, {
      headers: { Authorization: localStorage.getItem("cookie-token") },
    })
    .then((res) => {
      dispatch({
